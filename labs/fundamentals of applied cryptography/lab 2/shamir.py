...
def shamir() -> None:
    ...
    cprint(''.join([
        ...
        f'Ключи шифрования пользователя B: {keys_b}']), 'yellow')
    ...
    cprint('сообщение на первом шаге алгоритма: {}'.format(
        ''.join(get_text_from_ascii_list(C1))), 'yellow')
    C2 = [pow(C1_char, keys_b[0], p) for C1_char in C1]
    cprint('сообщение на втором шаге алгоритма: {}'.format(
        ''.join(get_text_from_ascii_list(C2))), 'yellow')
    C3 = [pow(C2_char, keys_a[1], p) for C2_char in C2]
    cprint('сообщение на третьем шаге алгоритма: {}'.format(
        ''.join(get_text_from_ascii_list(C3))), 'yellow')
    decrypted_message = [pow(C3_char, keys_b[1], p) for C3_char in C3]
    cprint('дешифрованное сообщение: {}'.format(
        ''.join(get_text_from_ascii_list(decrypted_message))), 'yellow')
